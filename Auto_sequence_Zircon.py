...
input_col =[[sg.T('元素标样',justification='c',size= (10,1)),sg.T('数量(自动生成)',justification='c',size= (10,1))],
            [sg.Input('SRM 610', key= 'Elemstand',justification='r',size= (10,1)),sg.Input('11',background_color='yellow', key= 'nElemstand',justification='r',size= (10,1))],
            [sg.T('年龄标样',justification='c',size= (10,1)),sg.T('数量',justification='c',size= (10,1))],
            [sg.Input('91500', key= 'Agestand',justification='r',size= (10,1)),sg.Input('22',background_color='yellow', key= 'nAgestand',justification='r',size= (10,1))],
            [sg.T('年龄副标',justification='c',size= (10,1)),sg.T('数量',justification='c',size= (10,1))],
            [sg.Input('PLE', key= 'Agemonst',justification='r',size= (10,1)),sg.Input('20',background_color='yellow', key= 'nAgemonst',justification='r',size= (10,1))],
            [sg.T('样品总数')],
            ...
            [sg.Button('Generate')],
            [sg.FileSaveAs('Export Table',key='Export_Table',enable_events = True,disabled=True,file_types = (('MS EXCEL', '.xlsx'),))]  # TODO: better names
            ]
...
#
    if event == 'Generate':
        ...
        #空序列用来装所有的样品明
        top_standards = [Elem_stand, Age_stand, Age_stand]
        all_sqz = []
        if number_last_sqz == 0: # 当余数等于0时候,10个满序列
            for i in range(number_sqz):


                first_five = [Samp_start+str(j+i*sz_sq+1).zfill(3) for j in range(semi_szsq)]  #前一半
                mid = [Age_monst,Age_monst]
                last_five = [Samp_start+str(j+i*sz_sq+semi_szsq+1).zfill(3) for j in range(semi_szsq)]  #后一半

                sqz_sample_name = top_standards+ first_five+mid+last_five    #一小组
            ...
            #多余的样品
            if number_last_sqz <= 2:  # 如果是1或者2，那么就放在后面一段上
                first_duan = semi_szsq
                last_duan = semi_szsq+number_last_sqz
            elif number_last_sqz == 3:  # 如果是3，那么就放在前面小段放1，后面小段放2
                first_duan = semi_szsq+1
                last_duan = semi_szsq+2

            else: # 如果是4，那么就放在前面小段放2，后面小段放2
                first_duan = semi_szsq+2
                last_duan = semi_szsq+2

            first_five = [Samp_start + str(j + i * sz_sq + 1).zfill(3) for j in range(first_duan)]  # 前xiao
            mid = [Age_monst, Age_monst]
            last_five = [Samp_start + str(j + i * sz_sq + 1+first_duan).zfill(3) for j in range(last_duan)]  # 后五个
            sqz_sample_name = top_standards + first_five + mid + last_five  # 一小组
            ...
            for i in range(number_sqz):
                ...
                all_sqz.extend(sqz_sample_name)
            #额外的一小列
            extra_sample =top_standards+[Age_monst]+[Samp_start + str(j + (number_sqz) * sz_sq + 1).zfill(3) for j in range(number_last_sqz)]+[Age_monst]
            all_sqz.extend(extra_sample)

        end_standards = [Age_stand,Age_stand,Elem_stand]
